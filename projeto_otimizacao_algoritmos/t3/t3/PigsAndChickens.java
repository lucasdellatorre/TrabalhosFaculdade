public class PigsAndChickens {
    ...
    public int countPlays() {
        return countPlays( new int[matrixLength][matrixLength], 0, 0, 0, 0, 0 );
    }
    ...
    private int chickens( int[][] board, int chicken, int count, int row, int col ) {
        ...
        if ( chicken == chickensCounter ) return ++count; 
    ...
    private boolean possible( int[][] board, int row, int col ) {
        if ( board[row][col] != Animals.EMPTY.getValue() ) return false;
        ...
        /* verifica diagonal esquerda baixo */  
